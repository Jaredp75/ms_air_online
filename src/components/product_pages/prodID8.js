...
class prodID8 extends Component {
  ...
  componentWillMount(){
    fetch( 'https://www.msaironline.com/qa1/api/product.php?id=8' )
      ...
      .then( ({product: items}) => this.setState({items}))
  }
  render() {
    ...
    return (
      <div>
        {items.map(item =>
          <h4 key={item.icon}>{item.icon}</h4>)}
        {items.map(item =>
          <h4 key={item.prodSku}>{item.prodSku}</h4>)}
        {items.map(item =>
          <h4 key={item.prodName}>{item.prodName}</h4>)}
        {items.map(item =>
          <h4 key={item.prodDesc}>{item.prodDesc}</h4>)}
        {items.map(item =>
          <h4 key={item.prodLongDesc}>{item.prodLongDesc}</h4>)}
          {items.map(item =>
            <h4 key={item.attributes.field}>{item.attributes.field}</h4>)}
